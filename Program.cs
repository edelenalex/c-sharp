...
// File: Program.cs
// This file uses IComparable to list library items in ceratin orders
...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        items.Sort(); // Sort - uses natural/ascending order
        WriteLine("Sorted list (natural/ascending order by Title):");
        foreach (LibraryItem i in items)
            WriteLine($"{i}\n");
        Pause();

        items.Sort(new DescendingByCopyRight()); // Sort - uses specified Comparer class

        WriteLine("Sorted list (descending natural order by Copyright Year) using Comparer:");
        foreach (LibraryItem i in items)
            WriteLine($"{i}\n");
        Pause();
